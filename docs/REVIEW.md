...
1. High: Starting a template can silently destroy an in-progress workout.
...
2. High: Unsaved edits are lost when editing an existing template and hopping to the exercise picker.
...
3. Medium: Adding an exercise to a template-backed workout can rewrite template order incorrectly.
...
4. Medium: `ConfirmDialog` checkbox state leaks across openings.
...
5. Medium: Active-workout validation is effectively disabled.
...
6. Low/Medium: Overnight workouts are attributed to the start date, not the finish date.
...
- `npm test`, `npm run build`, and `npm run lint` all passed.
- These are logic and state-management issues that the current tests are not covering yet, which matches the gaps listed in `TESTS.md`.
